...
class StoryRemoteDataSource @Inject constructor(
            ...
            try {
                ...
                val storyList = result.documents.map { document ->
                    ...
                    StoryRemoteModel(
                        ...
                        story_listen_time_in_millis = document.data["story_listen_time_in_millis"] as? Long ?: 0L,
                ...
                Log.e("StoryRemoteDataSource", "💥 Error fetching stories: ${e.message}")
    ...
    suspend fun fetchStoryAudioLocalizations(): Result<List<StoryAudioLanguageRemoteModel>> {
        return withContext(Dispatchers.IO) {
            try {
                Log.d("StoryRemoteDataSource", "🎵 Fetching story audio languages from Appwrite...")
                ...
                Log.d("StoryRemoteDataSource", "🎵 Successfully fetched ${storyAudioLanguageList} story audio language records")
                Result.success(storyAudioLanguageList)

            } catch (e: Exception) {
                Log.e("StoryRemoteDataSource", "💥 Error fetching story audio languages: ${e.message}")
                Result.failure(e)
            }
        }
    }
