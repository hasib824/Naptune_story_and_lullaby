...
fun StoryScreen(
    ...
    // ✅ Get currently playing story ID from ViewModel (which gets it from MusicController)
    ...
    // Accessed via: (uiState as? StoryUiState.Content)?.isPremium ?: false

    // ✅ Debug logs
    Log.d("StoryScreen", "📚 Currently playing story ID: $currentlyPlayingStoryId")

    Log.d("StoryScreen", "🎬 StoryScreen composable called")

    // ✅ ARCHITECTURE FIX: Ad initialization moved to ViewModel init block
    // Ads are only initialized for free users in StoryViewModel
    LaunchedEffect(Unit) {
        Log.d("StoryScreen", "🚀 StoryScreen composed - ViewModel handles ad initialization")
    }

    val uiState = viewModel.uiState.collectAsStateWithLifecycle().value
    Log.d("StoryScreen", "📊 Current UI State: $uiState")
        ...
        when (uiState) {
            is StoryUiState.Content -> {
                ...
                Column(
                    ...
                    Log.d("StoryScreen", "✅ Showing content with ${uiState.storyList.size} stories")
                    ...
                    // ✅ Determine which stories to show based on filter state
                    ...
                    Box(modifier = Modifier.fillMaxSize()) {
                        MakeStoryList(
                            ...
                            onStoryItemClick = { story ->
                                ...
                                viewModel.trackStorySelected(story, "listen")
                                // ✅ Call the original callback - MusicController will handle the rest
                                Log.d(
                                    "StoryScreen",
                                    "📚 Playing story: ${story.storyName} (ID: ${story.documentId})"
                                )
                            ...
                            // ✅ Pass currently playing ID from ViewModel
                            ...
                            // ✅ NEW: Pass session-unlocked IDs to hide ad badge for unlocked items
                        ...
                        RewardVideoBottomSheet(
                            ...
                            onWatchClick = {
                                ...
                                selectedStoryForAd?.let { story ->
                                    Log.d(
                                        "StoryScreen",
                                        "🎁 Rewarded ad triggered for story: ${story.storyName}"
                                    )
                                    if (context is android.app.Activity) {
                                        ...
                                        Log.e(
                                            "StoryScreen",
                                            "❌ Context is not an Activity, cannot show rewarded ad"
                                        )
                                    }
                                }
                                showWatchAdSheet = false
            ...
            StoryUiState.IsLoading -> {
                Log.d("StoryScreen", "⏳ Showing loading...")
                LoadingIndicator()
            }

            is StoryUiState.Error -> {
                Log.e("StoryScreen", "❌ Showing error: ${uiState.message}")
                Column(
                    ...
                    Button(
                        onClick = {
                            Log.d("StoryScreen", "🔄 Retry button clicked")
